...
__global__ void kernel_compute_vertex_map(const cv::cuda::PtrStepSz<float> depth_map, cv::cuda::PtrStepSz<float3> vertex_map, cv::cuda::PtrStepSz<int> valid_vertex_mask, const CameraIntrinsics camera_params, const float max_depth){
...
void compute_vertex_map(const GpuMat& filtered_depth_map, GpuMat& vertex_map, GpuMat& valid_vertex_mask, const CameraIntrinsics camera_params, const float max_depth){
    int threads = 32;
    ...
    kernel_compute_vertex_map<<< M , T >>>(filtered_depth_map, vertex_map, valid_vertex_mask, camera_params, max_depth);
