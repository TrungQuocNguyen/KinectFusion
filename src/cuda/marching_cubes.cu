...
#include <cuda/marching_cubes_tables.cuh>
                ...
                const int x, const int y, const int z, short& weight)
{
    ...
    return static_cast<float>(voxel_tuple.x) * INV_SHORT_MAX;
}
...
//##### HOST FUNCTIONS #####
SurfaceMesh marching_cubes(const TSDFData& volume, const int triangles_buffer_size)
{
    ...
    get_occupied_voxels_kernel<<<blocks, threads>>>(volume.tsdf, volume.volume_size,
    ...
    return SurfaceMesh { vertex_output, total_vertices, total_vertices / 3 };
}
